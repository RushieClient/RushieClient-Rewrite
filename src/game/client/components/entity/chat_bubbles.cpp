//Chat bubbles from https://github.com/FoxNet-DDNet/Entity-Client-DDNet
...
void CChatBubbles::UpdateBubbleOffsets(int ClientId, float InputBubbleHeight)
{
	...
	for(CBubble &Bubble : m_avChatBubbles[ClientId])
	{
		...
		Offset += BoundingBox.m_H + g_Config.m_RcChatBubbleSize + MarginBetween;
	}
}
...
void CChatBubbles::RenderCurInput(float y)
{
	...
	if(TextContainerIndex.Valid())
	{
		...
		Position.x -= BoundingBox.m_W / 2.0f + g_Config.m_RcChatBubbleSize / 15.0f;
		...
		Graphics()->DrawRect(Position.x - FontSize / 2.0f, TargetY - FontSize / 2.0f,
			...
			ColorRGBA(0.0f, 0.0f, 0.0f, 0.15f), IGraphics::CORNER_ALL, g_Config.m_RcChatBubbleSize / 4.5f);
...
{
	const float ShowTime = g_Config.m_RcChatBubbleShowTime / 100.0f;
	const int FontSize = g_Config.m_RcChatBubbleSize;
	...
	for(CBubble &Bubble : m_avChatBubbles[ClientId])
	{
		...
		//if(!g_Config.m_RcChatBubblePushOut)
		...
		if(Bubble.m_TextContainerIndex.Valid())
		{
			...
			float x = Position.x - (BoundingBox.m_W / 2.0f + g_Config.m_RcChatBubbleSize / 15.0f);
			...
			Graphics()->DrawRect((x - FontSize / 2.0f), y - FontSize / 2.0f,
				BoundingBox.m_W + FontSize * 1.20f, BoundingBox.m_H + FontSize,
				BgColor, IGraphics::CORNER_ALL, g_Config.m_RcChatBubbleSize / 4.5f);
...
{
	const float FadeOutTime = g_Config.m_RcChatBubbleFadeOut / 100.0f;
	const float FadeInTime = g_Config.m_RcChatBubbleFadeIn / 100.0f;
	const float ShowTime = g_Config.m_RcChatBubbleShowTime / 100.0f;
...
{
	if(m_UseChatBubbles != g_Config.m_RcChatBubbles)
	{
		Reset();
		m_UseChatBubbles = g_Config.m_RcChatBubbles;
	}
...
void CChatBubbles::SetupTextCursor(CTextCursor &Cursor)
{
	...
	Cursor.m_FontSize = g_Config.m_RcChatBubbleSize;
