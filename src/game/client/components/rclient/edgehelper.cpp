...
{
	Console()->Register("rc_toggle_edge_info", "", CFGFLAG_CLIENT, ConToggleEdgeHelper, this, "Toggle edge info");
}
...
void CEdgeHelper::SetActive(bool Active)
{
	...
	if(m_Active)
	{
		if(!g_Config.m_RcEdgeInfoJump && !g_Config.m_RcEdgeInfoCords)
		{
...
void CEdgeHelper::RenderEdgeHelper()
{
	...
	if(g_Config.m_RcEdgeInfoCords)
		RenderEdgeHelperEdgeInfo(g_Config.m_RcEdgeInfoJump ? &EdgeInfo : &Base);
	if(g_Config.m_RcEdgeInfoJump)
		RenderEdgeHelperJumpInfo(g_Config.m_RcEdgeInfoCords ? &JumpInfo : &Base);
}
